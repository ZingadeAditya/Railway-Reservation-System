...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent" >
    ...
    <ul class="navbar-nav mr-auto" style="margin-left: 135px;">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle" href="#" id="meals" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          MEALS
        </a>
        <div class="dropdown-menu" aria-labelledby="meals">
           <a class="dropdown-item" href="ecatering.php">E-Catering</a>
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="menu.php">Standard Menu Rate</a>
        </div>
      </li>
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="service">
          ...
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="e-bedroll.html">E-Bedroll</a>
          <div class="dropdown-divider"></div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["number"])) {
    ...
    if (!preg_match("/^[0-9]{10}$/",$number)) {
      $numberErr = "Please Enter 10 digit PNR"; 
    }
  }  
}
...
?>
  ...
  <div class="container">
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
             ...
             <label class="lab" style="padding-top:30px;">Enter PNR number :</label>
             <input type="text" name="number" placeholder="Enter 10 digit PNR number" maxlength="10" style="width: 300px; height: 30px;" value="<?php echo $number;?>">
...
$sql = "select p.TrainNo,p.TrainName,tp.PnrNo,tp.PassengerName,tp.PassengerGender,tp.PassengerAge,tp.SourceStation,tp.DestStation,tp.TravelDate,tp.PnrStatus from train_pnr tp, traininfo p where tp.PnrNo = '".$_POST['number']."' and p.TrainId = tp.TrainId";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    
  echo " <p> <font face='VERDANA'  size='4rt' >Showing Results for Pnr Number : ".$_POST['number']."</font></p><br>";
   echo "<table id = 'searchpnr'>";
    echo "<tr><td style='font-weight: bold;'>TrainNo</td><td align='center' style='font-weight: bold;'>TrainName</td><td style='font-weight: bold;'>PNR NO</td><td style='font-weight: bold;'>PASSENGER NAME</td><td style='font-weight: bold;'>PASSENGER GENDER</td><td style='font-weight: bold;'>Passenger AGE </td> <td style='font-weight: bold;'> SOUCRE STATION </td> <td style='font-weight: bold;'> DESTINATION STATION </td> <td style='font-weight: bold;'> JOURNEY DATE </td> <td style='font-weight: bold;'> STATUS </td> ";
    while($row = $result->fetch_assoc()) {
        
        echo "<tr><td>" . $row["TrainNo"]. " <td width='100px'> " . $row["TrainName"]. " <td> " .$row["PnrNo"]. "<td> ".$row["PassengerName"]. "<td>" .$row["PassengerGender"]." <td>" .$row["PassengerAge"]." <td> " .$row["SourceStation"]." <td> " .$row["DestStation"]. " <td> ".$row["TravelDate"]."<td>".$row["PnrStatus"]."</td></tr>";
    }

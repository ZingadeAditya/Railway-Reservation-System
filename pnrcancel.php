...
$sql1 = "select p.TrainNo,p.TrainName,tp.PnrNo,tp.PassengerName,tp.PassengerGender,tp.PassengerAge,tp.SourceStation,tp.DestStation,tp.TravelDate from train_pnr tp, traininfo p where tp.PnrNo = '".$_POST['pnr']."' and p.TrainId = tp.TrainId ";
  ...
  echo " <p> <font face='VERDANA'  size='4rt' >Showing Results for Pnr Number : ".$_POST['pnr']."</font></p><br>";
   echo "<table id = 'searchpnr' border='2'>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<tr><td>" . $row["TrainNo"]. " <td width='100px'> " . $row["TrainName"]. " <td> " .$row["PnrNo"]. "<td> ".$row["PassengerName"]. "<td>" .$row["PassengerGender"]." <td>" .$row["PassengerAge"]." <td> " .$row["SourceStation"]." <td> " .$row["DestStation"]. " <td> ".$row["TravelDate"]."</tr></table>";
    }
// sql to delete a record
$sql = "DELETE FROM pnrtable  WHERE PnrNo = '".$pnr."' and UserName like '".$_SESSION['uname']."' ";
